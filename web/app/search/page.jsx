    ...
    <main className="min-h-screen bg-gray-950">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <Link href="/" className="text-xl sm:text-2xl font-bold text-white hover:text-blue-400">
            Oracle Cloud Tables
          </Link>
        </div>
        ...
        <form className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 px-4 sm:px-5 py-3 sm:py-4 text-base sm:text-lg bg-gray-900 border border-gray-700 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              autoFocus
            />
        ...
        {q && (
          <>
            {/* Results Summary */}
            <div className="flex flex-wrap gap-2 sm:gap-4 mb-4 sm:mb-6 border-b border-gray-800 pb-3 sm:pb-4">
              <span className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600/20 text-blue-400 rounded-lg text-sm sm:text-base">
                {results.tables.length} Tables
              </span>
              <span className="px-3 sm:px-4 py-1.5 sm:py-2 bg-green-600/20 text-green-400 rounded-lg text-sm sm:text-base">
                {groupedCols.length} Columns <span className="hidden sm:inline">({results.columns.length} occurrences)</span>
              </span>
            </div>
                ...
                <div className="grid gap-2">
                  {results.tables.map(t => (
                    <Link key={t.id} href={`/table/${t.name}`}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-gray-900 hover:bg-gray-800 border border-gray-800 hover:border-gray-700 rounded-lg p-3 sm:p-4 transition-colors gap-1.5 sm:gap-4">
                      <div className="min-w-0">
                        <span className="font-mono text-blue-400 text-sm sm:text-lg break-all">{t.name}</span>
                        {t.description && (
                          <p className="text-gray-500 text-xs sm:text-sm mt-0.5 sm:mt-1 line-clamp-1">{t.description}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        <span className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 bg-gray-800 text-gray-400 rounded">{t.module_name || 'HCM'}</span>
                        <span className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 bg-gray-800 text-gray-400 rounded">{t.type}</span>
                  ...
                  <div className="sm:hidden divide-y divide-gray-800">
                    {groupedCols.map((col, i) => (
                      <div key={i} className="p-3">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {col.tables.slice(0, 4).map((tbl, j) => (
                            <Link key={j} href={`/table/${tbl}`}
                              className="text-[10px] px-1.5 py-0.5 bg-blue-900/30 text-blue-400 hover:bg-blue-900/50 rounded">
                              {tbl}
                            </Link>
                          ))}
                          {col.tables.length > 4 && (
                            <span className="text-[10px] px-1.5 py-0.5 text-gray-500">
                              +{col.tables.length - 4} more
                            </span>
                          )}
                        </div>
                      </div>
